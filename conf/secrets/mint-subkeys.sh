...
#     conf/secrets/.sops.yaml (with that machine's public age key)
#
...
EXPIRE="${3:-0}" # 0 = never
NAME="cogs@${HOST}/gpg"
DIR="$(cd "$(dirname "$0")" && pwd)" # this conf/secrets/ dir (sops rules -- .sops.yaml -- live in it)
...
echo ">> wrote conf/secrets/${NAME}.sops (encrypted to cogs@${HOST})"
