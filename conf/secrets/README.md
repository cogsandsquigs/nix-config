...
```sh
./sops-stash.sh ~/api-token.txt cogs/api-token   # -> conf/secrets/cogs/api-token.sops
```

(`sops-stash.sh` only wraps `sops encrypt --input-type binary`. The `cogs/api-token` path picks the
recipient from `.sops.yaml`.)

Use it from your config (`conf/users/cogs/home.nix`):
...
./sops-stash.sh /tmp/sub.asc cogs@glorpbox/gpg             # -> conf/secrets/cogs@glorpbox/gpg.sops
rm -P /tmp/sub.asc                                             # wipe the temp copy (Linux: shred -u)
```

Wire it per machine in `conf/users/cogs/home.nix`, gated on whether the `.sops` file exists. Only
...
```sh
./mint-subkeys.sh <master-secret.asc> <host>   # -> conf/secrets/cogs@<host>/gpg.sops
```
