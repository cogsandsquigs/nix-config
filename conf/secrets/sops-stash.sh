...
#
# Encrypts <plaintext-file> into conf/secrets/<secret-path>.sops using sops (binary mode).
# Must be run from anywhere in the repo; finds conf/secrets/ automatically.
# The recipient key(s) are chosen by <secret-path> via the creation_rules in
...
SECRETS_DIR="$(git -C "$(dirname "$0")" rev-parse --show-toplevel)/conf/secrets"
[ -d "$SECRETS_DIR" ] || {
    echo "error: conf/secrets/ not found in repo root" >&2
    exit 1
}
...
echo "wrote conf/secrets/${SECRET}.sops"
