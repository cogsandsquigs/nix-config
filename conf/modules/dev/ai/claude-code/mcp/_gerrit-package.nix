# GerritCodeReview/gerrit-mcp-server as a stdio launcher. Wired up by ./gerrit.nix.
#
...
writeShellApplication {
    ...
    # ./gerrit.nix). Upstream wants a JSON config file, so translate at launch -- nothing in the
    # store.
    text = ''
