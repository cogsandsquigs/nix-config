#!/usr/bin/env python3
"""Roll every checks.json under a run dir into a compliance-vs-size table."""
import json
import sys
...
for s in sorted(run.glob("*/rep*/*/checks.json")):
    variant, sc = s.parts[-4], s.parts[-2]
...
print("# leash compression benchmark\n")
...
for variant in sorted(by_variant):
    ...
    print(f"| {variant} | {sizes.get(variant, '')} | {p}/{t} | {p / t:.0%} | "
          f"{', '.join(sorted(set(fails[variant]))) or '--'} |")
