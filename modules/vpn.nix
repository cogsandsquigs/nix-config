# VPN clients and profile management.
#
# The system halves used to be one classless file branching on the platform with `tools.conf.eachOs`.
# Splitting by class removes the branch: NixOS installs nothing here, and macOS wants Homebrew casks.
{
    home =
        {
            pkgs,
        ...
        in
        {
            ...
            config = lib.mkIf cfg.enable {
                ...
                # macOS: OpenVPN Connect has no programmatic import interface -- profiles must be
                # imported once by hand through the app UI. The sops secret is still decrypted and
                # available at its runtime path (tools.secrets.path); nothing to do here.
