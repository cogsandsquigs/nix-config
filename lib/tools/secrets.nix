{ root }:
let
    # A secret is addressed by (location, name): `location` is its audience folder under
    # `conf/secrets/` -- an identity "<user>@<host>" for one machine, or a bare "<user>" for that
    # user everywhere (see conf/secrets/.sops.yaml for how a folder resolves to recipients). Built
    # from the flake root the caller passes, so it resolves to the repo-root `conf/secrets/` no
    # matter who imports this file.
    secretFile = location: name: root + "/conf/secrets/${location}/${name}.sops";
